// SPDX-FileCopyrightText: 2025 Contributors to the CitrineOS Project
//
// SPDX-License-Identifier: Apache-2.0

/**
 * This file was automatically generated by json-schema-to-typescript.
 * DO NOT MODIFY IT BY HAND. Instead, modify the source JSONSchema file,
 * and run json-schema-to-typescript to regenerate this file.
 */

import { TariffGetStatusEnumType, TariffKindEnumType } from '../enums/index.js';
import type { OcppResponse } from '../../../../index.js';

export interface GetTariffsResponse extends OcppResponse {
  status: TariffGetStatusEnumType;
  statusInfo?: StatusInfoType | null;
  /**
   * @minItems 1
   */
  tariffAssignments?: [TariffAssignmentType, ...TariffAssignmentType[]] | null;
  customData?: CustomDataType | null;
}
/**
 * Element providing more information about the status.
 *
 */
export interface StatusInfoType {
  /**
   * A predefined code for the reason why the status is returned in this response. The string is case-insensitive.
   *
   */
  reasonCode: string;
  /**
   * Additional text to provide detailed information.
   *
   */
  additionalInfo?: string | null;
  customData?: CustomDataType | null;
}
/**
 * This class does not get 'AdditionalProperties = false' in the schema generation, so it can be extended with arbitrary JSON properties to allow adding custom data.
 */
export interface CustomDataType {
  vendorId: string;
  [k: string]: unknown;
}
/**
 * Shows assignment of tariffs to EVSE or IdToken.
 *
 */
export interface TariffAssignmentType {
  /**
   * Tariff id.
   *
   */
  tariffId: string;
  tariffKind: TariffKindEnumType;
  /**
   * Date/time when this tariff become active.
   *
   */
  validFrom?: string | null;
  /**
   * @minItems 1
   */
  evseIds?: [number, ...number[]] | null;
  /**
   * IdTokens related to tariff
   *
   *
   * @minItems 1
   */
  idTokens?: [string, ...string[]] | null;
  customData?: CustomDataType | null;
}
