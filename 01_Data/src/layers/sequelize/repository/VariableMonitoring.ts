...
import { CallAction, CrudRepository, OCPP2_0_1, OCPP2_0_1_CallAction, SystemConfig } from '@citrineos/base';
...
export class SequelizeVariableMonitoringRepository extends SequelizeRepository<VariableMonitoring> implements IVariableMonitoringRepository {
  ...
  async createOrUpdateByMonitoringDataTypeAndStationId(value: OCPP2_0_1.MonitoringDataType, componentId: string, variableId: string, stationId: string): Promise<VariableMonitoring[]> {
    return await Promise.all(
      value.variableMonitoring.map(async (variableMonitoring) => {
        ...
        await this.createVariableMonitoringStatus(OCPP2_0_1.SetMonitoringStatusEnumType.Accepted, OCPP2_0_1_CallAction.NotifyMonitoringReport, savedVariableMonitoring.get('databaseId'));
  ...
  async createVariableMonitoringStatus(status: OCPP2_0_1.SetMonitoringStatusEnumType, action: CallAction, variableMonitoringId: number): Promise<void> {
  ...
  async createOrUpdateBySetMonitoringDataTypeAndStationId(value: OCPP2_0_1.SetMonitoringDataType, componentId: string, variableId: string, stationId: string): Promise<VariableMonitoring> {
  ...
  async rejectAllVariableMonitoringsByStationId(action: CallAction, stationId: string): Promise<void> {
    await this.readAllByQuery({
      ...
      for (const variableMonitoring of variableMonitorings) {
        await this.createVariableMonitoringStatus(OCPP2_0_1.SetMonitoringStatusEnumType.Rejected, action, variableMonitoring.databaseId);
      }
    });
  }
  ...
  async updateResultByStationId(result: OCPP2_0_1.SetMonitoringResultType, stationId: string): Promise<VariableMonitoring> {
  ...
  async createEventDatumByComponentIdAndVariableIdAndStationId(event: OCPP2_0_1.EventDataType, componentId: string, variableId: string, stationId: string): Promise<EventData> {
