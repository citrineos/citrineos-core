...
import { IDeviceModelRepository, ILocalAuthListRepository, LocalListVersion, SendLocalList, VariableAttribute, VariableCharacteristics, } from "@citrineos/data";
import { LocalAuthListService } from "../../src/module/LocalAuthListService";
import { AuthorizationData, DataEnumType, UpdateEnumType } from "@citrineos/base";
...
describe('LocalAuthListService', () => {
    ...
    beforeEach(() => {
        ...
        localAuthListService = new LocalAuthListService(mockLocalAuthListRepository, mockDeviceModelRepository);
    });
    ...
    it('should persist SendLocalListRequest and return the SendLocalList, validating input arguments', async () => {
        ...
        const sendLocalListRequest = {
            ...
            localAuthorizationList: [{ idToken: { idToken: 'ID_TOKEN', type: 'Central' }, idTokenInfo: { status: 'Accepted' } }] as [AuthorizationData],
        ...
        const testMockVariableCharacteristics = Object.assign({}, baseMockVariableCharacteristics, { maxLimit: 10 });

        mockLocalAuthListRepository.readOnlyOneByQuery.mockResolvedValue(baseMockLocalListVersion);
        mockLocalAuthListRepository.createSendLocalListFromRequestData.mockResolvedValue(mockSendLocalList);
        mockDeviceModelRepository.findVariableCharacteristicsByVariableNameAndVariableInstance.mockResolvedValue(testMockVariableCharacteristics);
        mockDeviceModelRepository.readAllByQuerystring.mockResolvedValue([]);

        const result = await localAuthListService.persistSendLocalListForStationIdAndCorrelationIdAndSendLocalListRequest(stationId, correlationId, sendLocalListRequest);

        expect(result).toEqual(mockSendLocalList);
        expect(mockLocalAuthListRepository.createSendLocalListFromRequestData).toHaveBeenCalledWith(stationId, expectedCorrelationId, expectedUpdateType, newVersionNumber, sendLocalListRequest.localAuthorizationList);
    });

    it('should throw an error when SendLocalListRequest.versionNumber is less than or equal to 0', async () => {
        const sendLocalListRequest_0 = { versionNumber: 0, updateType: UpdateEnumType.Full };

        await expect(localAuthListService.persistSendLocalListForStationIdAndCorrelationIdAndSendLocalListRequest(stationId, correlationId, sendLocalListRequest_0))
            .rejects
            .toThrow('Version number 0 must be greater than 0, see D01.FR.18');


        const sendLocalListRequest_negative = { versionNumber: -1, updateType: UpdateEnumType.Full };

        await expect(localAuthListService.persistSendLocalListForStationIdAndCorrelationIdAndSendLocalListRequest(stationId, correlationId, sendLocalListRequest_negative))
            .rejects
            .toThrow('Version number -1 must be greater than 0, see D01.FR.18');
    });

    it('should throw an error when versionNumber is less than the current LocalListVersion', async () => {
        const sendLocalListRequest = { versionNumber: 1, updateType: UpdateEnumType.Full };
        ...
        await expect(localAuthListService.persistSendLocalListForStationIdAndCorrelationIdAndSendLocalListRequest(stationId, correlationId, sendLocalListRequest))
            .rejects
            .toThrow(`Current LocalListVersion for ${stationId} is 2, cannot send LocalListVersion 1 (version number must be higher)`);
    });
    ...
    it('should throw an error when there are duplicate idTokens in the localAuthorizationList', async () => {
        ...
        const sendLocalListRequest = {
            ...
            updateType: expectedUpdateType,
            localAuthorizationList: [
                { idToken: { idToken: 'ID_TOKEN', type: 'Central' }, idTokenInfo: { status: 'Accepted' } },
                { idToken: { idToken: 'ID_TOKEN', type: 'Central' }, idTokenInfo: { status: 'Blocked' } }
        ...
        await expect(localAuthListService.persistSendLocalListForStationIdAndCorrelationIdAndSendLocalListRequest(stationId, correlationId, sendLocalListRequest))
            .rejects
            .toThrow('Duplicated idToken in SendLocalList ["ID_TOKENCentral","ID_TOKENCentral"]');
    });
    ...
    it('should throw an error when updated list length exceeds maxLocalAuthListEntries', async () => {
        ...
        const sendLocalListRequest = {
            ...
            localAuthorizationList: [
                { idToken: { idToken: 'ID_TOKEN1', type: 'Central' }, idTokenInfo: { status: 'Accepted' } },
                { idToken: { idToken: 'ID_TOKEN2', type: 'Central' }, idTokenInfo: { status: 'Blocked' } }
            ] as [AuthorizationData, AuthorizationData]
        };
        ...
        const testMockVariableCharacteristics = Object.assign({}, baseMockVariableCharacteristics, { maxLimit: 1 });
        ...
        await expect(localAuthListService.persistSendLocalListForStationIdAndCorrelationIdAndSendLocalListRequest(stationId, correlationId, sendLocalListRequest))
            .rejects
            .toThrow('Updated local auth list length (2) will exceed max local auth list entries (1)');
    });
    ...
    it('should throw an error when getMaxLocalAuthListEntries returns null', async () => {
        ...
        mockDeviceModelRepository.findVariableCharacteristicsByVariableNameAndVariableInstance.mockResolvedValue(undefined); // No variable characteristics
        mockLocalAuthListRepository.createSendLocalListFromRequestData.mockResolvedValue(mockSendLocalList);

        await expect(localAuthListService.persistSendLocalListForStationIdAndCorrelationIdAndSendLocalListRequest(stationId, correlationId, sendLocalListRequest))
            .rejects
            .toThrow('Could not get max local auth list entries, required by D01.FR.12');
    });
    ...
    it('should throw an error when getMaxLocalAuthListEntries returns variable characteristics without maxLimit', async () => {
        ...
        mockDeviceModelRepository.findVariableCharacteristicsByVariableNameAndVariableInstance.mockResolvedValue(baseMockVariableCharacteristics);
        mockLocalAuthListRepository.createSendLocalListFromRequestData.mockResolvedValue(mockSendLocalList);
