...
export class Local implements ICertificateAuthorityClient {
    ...
    async getSignedCertificate(csrString: string, stationId?: string): Promise<string> {
        if (!stationId) {
            throw new Error('stationId is required');
        }
        const clientConnection: string = (await this._cache.get(
        ...
        return forge.pki.certificateToPem(this._createSignedCertificate(csr, rootCert, rootPrivateKey));
    }
