...
export class Hubject implements ICertificateAuthorityClient {
    ...
    constructor(config: SystemConfig) {
        ...
        this._baseUrl = config.modules.certificates?.certificateAuthority.hubject.baseUrl;
        this._tokenUrl = config.modules.certificates?.certificateAuthority.hubject.tokenUrl;
        this._isoVersion = config.modules.certificates?.certificateAuthority.hubject.isoVersion;
    }
    ...
    async getSignedCertificate(csr: string): Promise<string> {
        this._authorizationToken = this._authorizationToken || await this._getAuthorizationToken(this._tokenUrl);
        ...
        if (response.status !== 200) {
            throw new Error(`Get signed certificate response is unexpected: ${response.status}: ${await response.text()}`);
        }
    ...
    async getCACertificates(): Promise<string> {
        ...
        if (response.status !== 200) {
            throw new Error(`Get CA certificates response is unexpected: ${response.status}: ${await response.text()}`);
        }
    ...
    private async _getAuthorizationToken(tokenUrl: string): Promise<string> {
        return fetch(tokenUrl, { method: 'GET' }).then(response => {
            if (response.status !== 304) {
                throw new Error(`Get token response is unexpected: ${response.status}`);
            }
            return response.json();
        }).then(data => this._parseBearerToken(data.data)).catch(error => {
