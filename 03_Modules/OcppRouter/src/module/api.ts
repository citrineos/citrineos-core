  ...
  implements IAdminApi
{
  ...
  async putSubscription(
    ...
    if (request.body.onConnect) {
      ...
      this._logger.debug(
        `Added onConnect callback to ${request.body.url} for station ${request.body.stationId}`,
      );
    }
    if (request.body.onClose) {
      ...
      this._logger.debug(
        `Added onClose callback to ${request.body.url} for station ${request.body.stationId}`,
      );
    }
    if (request.body.onMessage) {
      this._module.addOnMessageCallback(
        async (identifier: string, message: string) => {
          if (
            identifier === request.body.stationId &&
            (!request.body.messageOptions?.regexFilter ||
              new RegExp(request.body.messageOptions.regexFilter).test(message))
          ) {
            return fetch(request.body.url, {
              method: 'POST',
              headers: {
      ...
      this._logger.debug(
        `Added onMessage callback to ${request.body.url} for station ${request.body.stationId}`,
      );
    }
    if (request.body.sentMessage) {
      this._module.addSentMessageCallback(
        async (identifier: string, message: string, error?: any) => {
          if (
            identifier === request.body.stationId &&
            (!request.body.messageOptions?.regexFilter ||
              new RegExp(request.body.messageOptions.regexFilter).test(message))
      ...
      this._logger.debug(
        `Added sentMessage callback to ${request.body.url} for station ${request.body.stationId}`,
      );
    }
    return;
  }
