...
describe('WebhookDispatcher', () => {
    ...
    describe('dispatchMessageReceived', () => {
        ...
        it('should send request when message matches filter', async () => {
            const subscription = aSubscription({onMessage: true, messageRegexFilter: 'Accepted'});
            givenSubscriptions(subscription);
            await givenRegisteredStations(subscription.stationId);
        ...
        it('should not send request when message does not match filter', async () => {
            ...
            await webhookDispatcher.dispatchMessageReceived(subscription.stationId, 'Rejected reservation');
    ...
    describe('dispatchMessageSent', () => {
        ...
        it('should send request when message matches filter', async () => {
            const subscription = aSubscription({sentMessage: true, messageRegexFilter: 'totalCost'});
            givenSubscriptions(subscription);
            await givenRegisteredStations(subscription.stationId);
    ...
    describe('scheduled job',  () => {
        ...
        it('should periodically pick up new subscriptions for registered stations', async () => {
            ...
            const newSubscription = aSubscription({stationId: subscription.stationId, sentMessage: true, messageRegexFilter: undefined});
            ...
            expect(subscriptionRepository.readAllByStationId).toHaveBeenCalledWith(subscription.stationId);
        ...
        it('should periodically remove deleted subscriptions for registered stations', async () => {
            ...
            const subscription = aSubscription({stationId: stationId, sentMessage: true, messageRegexFilter: undefined});
            const anotherSubscription = aSubscription({stationId: stationId, sentMessage: true, messageRegexFilter: undefined});
