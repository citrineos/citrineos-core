...
export class ReportingModuleApi extends AbstractModuleApi<ReportingModule> implements IReportingModuleApi {
  ...
  async getCustomReport(
    ...
    // return error
    const bytesPerMessageGetReport = await this._module._deviceModelService.getBytesPerMessageByComponentAndVariableInstanceAndStationId(this._componentDeviceDataCtrlr, CallAction.GetReport, identifier);
    ...
    const componentVariables = request.componentVariable as ComponentVariableType[];
    ...
    for (const [index, batch] of getBatches(componentVariables, itemsPerMessageGetReport).entries()) {
      try {
        const batchResult = await this._module.sendCall(identifier, tenantId, CallAction.GetReport, {
          componentVariable: batch,
          componentCriteria: request.componentCriteria,
          requestId: request.requestId
        } as GetReportRequest, callbackUrl);
        confirmations.push({
  ...
  async getMonitoringReport(identifier: string, tenantId: string, request: GetMonitoringReportRequest, callbackUrl?: string): Promise<IMessageConfirmation> {
    const componentVariable = request.componentVariable as ComponentVariableType[];
    ...
    for (const [index, batch] of getBatches(componentVariable, itemsPerMessageGetReport).entries()) {
      try {
        const batchResult = await this._module.sendCall(identifier, tenantId, CallAction.GetMonitoringReport, {
          componentVariable: batch,
          monitoringCriteria: monitoringCriteria,
          requestId: request.requestId
        } as GetMonitoringReportRequest, callbackUrl);
        confirmations.push({
          success: batchResult.success,
          batch: `[${index}:${index + batch.length}]`,
          message: `${batchResult.payload}`,
