...
const additionalConverters: ISchemaConverters = {
  [IS_ENUM]: (meta: ValidationMetadata, _: IOptions) => {
    ...
    const enumName = Reflect.getMetadata('isEnum', (meta.target as any).prototype, meta.propertyName);
    ...
    const isOptional = Reflect.getMetadata('isOptional', (meta.target as any).prototype, meta.propertyName);
    if (isOptional) {
      return {
        oneOf: [
          {
  ...
  [ValidationTypes.NESTED_VALIDATION]: (meta: ValidationMetadata, options: IOptions) => {
    if (typeof meta.target === 'function') {
      ...
      if (isOptional) {
        ...
        if (schema && schema.$ref) {
          let obj: SchemaObject | ReferenceObject = {$ref: schema.$ref};
          anyOf.unshift(obj);
        }
        if (anyOf.length === 1) {
          return {
...
const getIsArray = (meta: ValidationMetadata): boolean => {
  ...
  const validations = metadataStorage
    .getTargetValidationMetadatas(meta.target as any, (meta.target as any).name, false, false);
