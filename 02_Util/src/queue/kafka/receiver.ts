...
export class KafkaReceiver extends AbstractMessageHandler implements IMessageHandler {
    ...
    constructor(
        ...
        this._client = new Kafka({
            brokers: this.configService?.systemConfig.util.messageBroker.kafka?.brokers || [],
            ssl: true,
            sasl: {
                mechanism: 'plain',
                username: this.configService?.systemConfig.util.messageBroker.kafka?.sasl.username || "",
                password: this.configService?.systemConfig.util.messageBroker.kafka?.sasl.password || ""
            }
        });

        this._topicName = `${this.configService?.systemConfig.util.messageBroker.kafka?.topicPrefix}-${this.configService?.systemConfig.util.messageBroker.kafka?.topicName}`;
