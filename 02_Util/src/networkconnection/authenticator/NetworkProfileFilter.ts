...
export class NetworkProfileFilter extends AuthenticatorFilter {
    ...
    private async _isConfigurationSlotAllowed(identifier: string, securityProfile: number) {
        const r = await this._deviceModelRepository
        ...
        if (r && r[0]) {
            ...
            if (configurationSlotsString && configurationSlotsString.trim() !== '') {
                ...
                const configurationSlotsArray = configurationSlotStringsArray.map(configurationSlotString => parseInt(configurationSlotString, 10))
                    .filter(configurationSlotId => {
                        if (isNaN(configurationSlotId)) {
                            this._logger.error("NetworkConfigurationPriority elements must be integers: " + configurationSlotsString);
                            return false;
                        } else {
                            return true;
                        }
                ...
                if (configurationSlotsArray.length == 0) {
                    ...
                    for (const configurationSlot of configurationSlotsArray) {
                        const chargingStationNetworkProfile = await ChargingStationNetworkProfile.findOne({ where: { stationId: identifier, configurationSlot: configurationSlot } });
