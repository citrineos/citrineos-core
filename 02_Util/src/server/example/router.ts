  ...
  async handle(
    ...
    if (message.state === MessageState.Response) {
      if (message.payload instanceof OcppError) {
        ...
        await this._centralSystem.sendCallError(
        ...
        await this._centralSystem.sendCallResult(
          message.context.stationId,
          callResult,
        );
      }
...
export class OcppMessageRouter implements IMessageRouter {
  ...
  async registerConnection(client: IClientConnection): Promise<boolean> {
    const requestSubscription = await this.handler.subscribe(
      client.identifier,
      undefined,
      {
    ...
    const responseSubscription = await this.handler.subscribe(
      client.identifier,
      undefined,
      {
        stationId: client.identifier,
  ...
  async handleMessageApiCallback(message: IMessage<OcppError>): Promise<void> {
    const url: string | null = await this._cache.get(
      message.context.correlationId,
      this.CALLBACK_URL_CACHE_PREFIX + message.context.stationId,
    );
    if (url) {
      await fetch(url, {
        method: "POST",
