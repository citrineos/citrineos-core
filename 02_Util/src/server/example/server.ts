 ...
 */
export class CentralSystemImpl
  extends AbstractCentralSystem
  implements ICentralSystem
{
  /**
   * Fields
   */
  ...
  constructor(
    ...
    this._deviceModelRepository =
      ...
      new DeviceModelRepository(this._config, this._logger);
    ...
    this._config.util.networkConnection.websocketServers.forEach((wsServer) => {
          ...
          _httpServer = https.createServer(
            {
              ...
              cert: fs.readFileSync(
                wsServer.tlsCertificateChainFilepath as string,
              ),
              ca: fs.readFileSync(
                wsServer.mtlsCertificateAuthorityRootsFilepath as string,
              ),
              requestCert: true,
      ...
      let _socketServer = new WebSocketServer({
        ...
        handleProtocols: (protocols, req) =>
          this._handleProtocols(protocols, req, wsServer.protocol),
      ...
      _socketServer.on(
        ...
        (ws: WebSocket, req: http.IncomingMessage) =>
          this._onConnection(ws, wsServer.pingInterval, req),
      );
      ...
      _httpServer.listen(wsServer.port, wsServer.host, () => {
        this._logger.info(
          `WebsocketServer running on ${protocol}://${wsServer.host}:${wsServer.port}/`,
        );
      });
  ...
  onCallResult(connection: IClientConnection, message: CallResult): void {
    ...
    this._cache
      .get<string>(connection.identifier, CacheNamespace.Transactions)
      .then((cachedActionMessageId) => {
        ...
        return {
          action,
          ...this._validateCallResult(connection.identifier, action, message),
  ...
  private _handleProtocols(
    ...
    if (protocols.has(wsServerProtocol)) {
      ...
      // Register client
      const registered = this._cache.setSync(
        clientConnection.identifier,
        JSON.stringify(instanceToPlain(clientConnection)),
        CacheNamespace.Connections,
      );
      if (!registered) {
        this._logger.fatal(
          "Failed to register websocket client",
          identifier,
          clientConnection,
        );
        return false;
  ...
  private _onConnection(
    ...
    const clientConnection = this._cache.getSync<ClientConnection>(
      identifier,
      CacheNamespace.Connections,
      () => ClientConnection,
    );
   ...
   */
  private _getClientConnection(
    identifier: string,
  ): Promise<IClientConnection | null> {
    return this._cache.get<ClientConnection>(
      identifier,
      CacheNamespace.Connections,
      () => ClientConnection,
    );
