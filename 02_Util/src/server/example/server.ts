...
export class CentralSystemImpl extends AbstractCentralSystem implements ICentralSystem {
  ...
  constructor(
    ...
    super(config, logger, cacheService?.cache, ajv);

    // Initialize router before socket server to avoid race condition
    this._router = new OcppMessageRouter(cacheService?.cache as ICache,
      sender || new RabbitMqSender(config, logger),
      handler || new CentralSystemMessageHandler(this, logger));
  ...
  onCall(connection: IClientConnection, message: Call): void {
    ...
    this._onCallIsAllowed(action, connection.identifier)
      ...
      return this.cacheService?.cache?.setIfNotExist(connection.identifier, `${action}:${messageId}`, CacheNamespace.Transactions, this._config.maxCallLengthSeconds);
    }).catch(error => {
      ...
      if (error instanceof OcppError) {
        this.sendCallError(connection.identifier, error.asCallError());
        this.cacheService?.cache?.remove(connection.identifier, CacheNamespace.Transactions);
      }
    });
  }
  ...
  onCallResult(connection: IClientConnection, message: CallResult): void {
    ...
    this.cacheService?.cache?.get<string>(connection.identifier, CacheNamespace.Transactions)
      .then(cachedActionMessageId => {
        this.cacheService?.cache?.remove(connection.identifier, CacheNamespace.Transactions); // Always remove pending call transaction
  ...
  async sendCall(identifier: string, message: Call): Promise<boolean> {
    ...
    if (await this._sendCallIsAllowed(identifier, message)) {
      if (await this.cacheService?.cache?.setIfNotExist(identifier, `${action}:${messageId}`,
  ...
  async sendCallResult(identifier: string, message: CallResult): Promise<boolean> {
    const messageId = message[1];
    const cachedActionMessageId = await this.cacheService?.cache?.get<string>(identifier, CacheNamespace.Transactions);
    if (!cachedActionMessageId) {
    ...
    if (cachedMessageId === messageId) {
      ...
      return Promise.all([
        ...
        this.cacheService?.cache?.remove(identifier, CacheNamespace.Transactions)
  ...
  private _onCallIsAllowed(action: CallAction, identifier: string): Promise<boolean> {
    return (this.cacheService?.cache?.exists(action, identifier) as Promise<boolean>).then(blacklisted => !blacklisted);
  }
  ...
  private _handleProtocols(protocols: Set<string>, req: http.IncomingMessage, wsServerProtocol: string) {
    ...
    if (protocols.has(wsServerProtocol)) {
      ...
      // Register client
      const registered = this.cacheService?.cache?.setSync(clientConnection.identifier, JSON.stringify(instanceToPlain(clientConnection)), CacheNamespace.Connections);
      if (!registered) {
  ...
  private _onConnection(ws: WebSocket, pingInterval: number, req: http.IncomingMessage): void {
    ...
    const clientConnection = this.cacheService?.cache?.getSync<ClientConnection>(identifier, CacheNamespace.Connections, () => ClientConnection);
    if (!clientConnection) {
  ...
  private _registerWebsocketEvents(identifier: string, ws: WebSocket, pingInterval: number): void {
    ...
    ws.onerror = (event: ErrorEvent) => {
      ...
      this._getClientConnection(identifier).then(clientConnection => {
        if (clientConnection) {
          ...
          this.cacheService?.cache?.set(clientConnection.identifier, JSON.stringify(instanceToPlain(clientConnection)), CacheNamespace.Connections);
        }
      });
    ...
    ws.once("close", () => {
      ...
      this.cacheService?.cache?.remove(identifier, CacheNamespace.Connections);
    ...
    ws.on("pong", () => {
      ...
      this._getClientConnection(identifier).then(clientConnection => {
        if (clientConnection) {
          clientConnection.isAlive = true;
          this.cacheService?.cache?.set(clientConnection.identifier, JSON.stringify(instanceToPlain(clientConnection)), CacheNamespace.Connections).then(() => {
  ...
  private _getClientConnection(identifier: string): Promise<IClientConnection | null> {
    return this.cacheService?.cache?.get<ClientConnection>(identifier, CacheNamespace.Connections, () => ClientConnection) as Promise<IClientConnection | null>;
  }
  ...
  private async _sendCallIsAllowed(identifier: string, message: Call): Promise<boolean> {
    const status = await this.cacheService?.cache?.get<string>(BOOT_STATUS, identifier);
