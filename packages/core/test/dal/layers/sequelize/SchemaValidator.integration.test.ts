...
//
// The schema here is produced by `sequelize.sync()`, so it is by construction
// exactly what the models declare. That makes this a test of the validator's
// type canonicalization rather than of the migrations: any finding at all means
// the validator and Postgres disagree about a column type this codebase
// actually uses, which would be a false positive at startup.
//
...
describe('validateSchema detects drift', () => {
  ...
  it('flags a column narrowed below what the models declare', async () => {
    ...
    try {
      ...
      await sequelizeInstance.query('ALTER TABLE "Boots" ALTER COLUMN "status" TYPE varchar(255)');
    }
  });
