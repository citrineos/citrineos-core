...
import { AbstractHandler, type AbstractHandlerDependencies, AsResponseHandler, type IMessage, type IOcppSender, OCPP2_request_types, OCPP2_response_types } from '@citrineos/base';
import { ChargingStationSequenceTypeEnum, DisplayMessageStatusEnum, type DisplayMessageStatusEnumType, EventGroup, type HandlerProperties, OCPP_2_VER_LIST, OCPP_CallAction } from '@citrineos/types';
import type { IMessageInfoRepository } from '@dal/interfaces/repositories.js';
import { IdGenerator } from '@util/index.js';

@AsResponseHandler(OCPP_2_VER_LIST, OCPP_CallAction.SetDisplayMessage)
export class SetDisplayMessageResponseOcpp2Handler extends AbstractHandler {
  ...
  async handle(
    ...
    this._logger.debug(
      this.createHandlerReceivedMessageLog('SetDisplayMessageResponse'),
      message,
      props,
    );
