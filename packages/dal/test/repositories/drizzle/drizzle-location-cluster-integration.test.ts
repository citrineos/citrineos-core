...
// Connectors, which do not exist yet) and one EVSE. The Evse, Connector and
// StatusNotification models all lost their stationId BeforeCreate hook when the
// lookup moved into the repositories, so the FK is passed in explicitly.
...
// stationId is what the (stationId, connectorId) unique index keys on, so it has
// to be set for that constraint to bite — NULLs compare distinct in Postgres.
...
describe('DrizzleConnectorRepository', () => {
  ...
  it('updateById rejects a duplicate stationId/connectorId pair', async () => {
    const repo = new DrizzleConnectorRepository(deps());
    const { station, typeA, typeB, evse } = await aCommissionedStation(TENANT);
    await aConnector(TENANT, station.id, evse.id, typeA.databaseId, 1);
    const second = await aConnector(TENANT, station.id, evse.id, typeB.databaseId, 2);
...
describe('location cluster row-to-DTO mappers', () => {
  ...
  it('toConnectorDto maps a null timestamp to empty string and exposes tariffId', () => {
    ...
    // The tariff relation is never populated, so the FK is the only way to reach it.
    expect(dto.tariffId).toBe(42);
    expect(dto.tariff).toBeUndefined();
  });
  ...
  it('toLatestStatusNotificationDto defaults a null pointer to 0', () => {
