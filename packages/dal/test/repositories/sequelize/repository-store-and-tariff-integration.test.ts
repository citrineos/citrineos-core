...
// logger and sequelize instance. CITRINEOS_USE_DRIZZLE=true swaps nineteen of
// them to Drizzle implementations; those only build a lazy pg pool from config
// at construction, so both branches are covered here. On the sequelize side the
// whole Location cluster (station/evse/connector/status notification) is one
// SequelizeLocationRepository instance aliased onto five properties.
...
// Evse/Connector no longer resolve stationId from ocppConnectionName in a
// BeforeCreate hook -- that moved to the repository layer (resolveStationId) --
// so a bare Model.create must carry the FK itself.
...
describe('RepositoryStore', () => {
  ...
  // Outside the drizzle branch station, evse, connector and status notification
  // reads are served by the Location aggregate itself, not separate instances.
  it('aliases the location cluster onto one repository instance by default', () => {
    withDrizzleEnv(undefined, () => {
      const store = makeStore();

      expect(store.chargingStationRepository).toBe(store.locationRepository);
      expect(store.evseRepository).toBe(store.locationRepository);
      expect(store.connectorRepository).toBe(store.locationRepository);
      expect(store.statusNotificationRepository).toBe(store.locationRepository);
    });
  });
  ...
  it('wires the switchable repositories to drizzle implementations when CITRINEOS_USE_DRIZZLE=true', () => {
    withDrizzleEnv('true', () => {
      ...
      expect(store.evseRepository).toBeInstanceOf(DrizzleEvseRepository);
      expect(store.installCertificateAttemptRepository).toBeInstanceOf(
        DrizzleInstallCertificateAttemptRepository,
      );
      expect(store.installedCertificateRepository).toBeInstanceOf(
      ...
      // The Location cluster is split into one Drizzle repository per role here,
      // so these are distinct instances rather than aliases of one another.
      expect(store.locationRepository).toBeInstanceOf(DrizzleLocationRepository);
      expect(store.locationRepository).not.toBe(store.chargingStationRepository);
      expect(store.messageInfoRepository).toBeInstanceOf(DrizzleMessageInfoRepository);
      ...
      expect(store.statusNotificationRepository).toBeInstanceOf(
        DrizzleStatusNotificationRepository,
      );
