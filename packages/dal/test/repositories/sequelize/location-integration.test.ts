...
// Evse, Connector and StatusNotification no longer resolve their stationId FK in a
// BeforeCreate hook — the repository resolves it through resolveStationId() before
// writing the row, so fixtures built straight from the models pass stationId themselves.
...
describe('SequelizeLocationRepository', () => {
  ...
  describe('addStatusNotificationToChargingStation', () => {
    ...
    it('a newer notification for the same evse and connector replaces the latest entry', async () => {
      ...
      await repo.addStatusNotificationToChargingStation(
        TENANT_A,
        STATION_NAME,
        aStatusNotification(connector1.id, { connectorStatus: 'Available' }),
      );
    ...
    it('notifications for different connectors keep separate latest entries', async () => {
      ...
      await repo.addStatusNotificationToChargingStation(
        TENANT_A,
        STATION_NAME,
        aStatusNotification(connector1.id),
      );
      await repo.addStatusNotificationToChargingStation(
        TENANT_A,
        STATION_NAME,
        aStatusNotification(connector2.id, { connectorStatus: 'Faulted' }),
      );
  ...
  // Replaces updateAllConnectorsByQuery: the raw Sequelize where-clause parameter
  // was swapped for the stationId its only caller passed.
  describe('updateAllConnectorsByStationId', () => {
    it('updates every connector of the station and honours the tenant scope', async () => {
      const { station, connector1, connector2 } = await aCommissionedStation();
      const repo = makeRepo();

      const updated = await repo.updateAllConnectorsByStationId(TENANT_A, station.id, {
        status: 'Unavailable',
      });
      const forB = await repo.updateAllConnectorsByStationId(TENANT_B, station.id, {
        status: 'Available',
      });

      expect(updated.map((c) => c.id).sort((a, b) => a - b)).toEqual([
        connector1.id,
        connector2.id,
      ]);
      expect(updated.map((c) => c.status)).toEqual(['Unavailable', 'Unavailable']);
      expect(forB).toHaveLength(0);
      const rows = await Connector.findAll({ order: [['id', 'ASC']] });
      expect(rows.map((r) => r.status)).toEqual(['Unavailable', 'Unavailable']);
    });

    it('leaves connectors of another station alone', async () => {
      const { station } = await aCommissionedStation();
      ...
      expect((await Connector.findByPk(otherConnector.id))!.status).toBe('Available');
    });
  });
