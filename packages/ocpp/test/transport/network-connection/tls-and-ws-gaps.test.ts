...
import { OCPP2_1, type SystemConfig, type WebsocketServerConfig } from '@citrineos/types';
...
describe('CertificateUtil gaps', () => {
  ...
  describe('createOcspRequest', () => {
    const ocspRequestData: OCPP2_1.OCSPRequestDataType = {
    ...
    it('encodes CertID from precomputed hashes, wrapped in an OCSPRequest', () => {
      const actualResult = createOcspRequest(ocspRequestData);

      // OCSPRequest > tbsRequest > requestList > Request > CertID(sha256 OID, hashes, serial)
      expect(actualResult.getEncodedHex()).toBe(
        [
          '30233021301f301d301b300d06096086480165030402010500',
          '0402aabb',
          '0402ccdd',
          '02021f2e',
        ].join(''),
    ...
    it('lowercases the hash algorithm', () => {
      ...
      expect(parseOcspRequestHex(actualResult.getEncodedHex())).toEqual([
        { alg: 'sha256', issname: 'aabb', isskey: 'ccdd', sbjsn: '1f2e' },
      ]);
    });
  });
