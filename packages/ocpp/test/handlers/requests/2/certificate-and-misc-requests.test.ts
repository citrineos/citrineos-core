...
describe('GetCertificateStatusRequestOcpp2Handler', () => {
  ...
  // sendOCSPRequest hands back the responder DER as hex; the handler base64-encodes it into
  // ocspResult. 'MAMKAQCAgYKD' is base64(hex '30030a010080818283').
  const responderDerHex = '30030a010080818283';
  const responderDerBase64 = 'MAMKAQCAgYKD';
  ...
  it('sends Accepted with the responder DER base64 encoded', async () => {
    ...
    mockSendOCSPRequest.mockResolvedValue(responderDerHex);
...
describe('VatNumberValidationRequestOcpp21Handler', () => {
  ...
  // company is an optional AddressType, so a provider null leaves the field off the response
  // rather than sending null.
  it('sends Rejected and omits company when the provider finds nothing', async () => {
    ...
    expect(getVat).toHaveBeenCalledOnce();
    const response = sentResponse<OCPP2_1.VatNumberValidationResponse>(ocppSender);
  ...
  it('sends Rejected and omits company when no VAT provider is configured', async () => {
