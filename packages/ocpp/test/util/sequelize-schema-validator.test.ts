...
/**
 * The comparison logic is exercised through validateSequelizeSchema() against a faked
...
async function findingsFor(models: any[], rows: FakeColumn[]): Promise<SchemaFinding[]> {
  ...
  const report = await validateSequelizeSchema(sequelize);
...
describe('SequelizeSchemaValidator', () => {
  // --- Introspection query -----------------------------------------------------

  describe('validateSequelizeSchema introspection', () => {
    it('reads the whole schema in a single query, parameterised by schema name', async () => {
      ...
      await validateSequelizeSchema(sequelize, { schema: 'tenant_a' });
    ...
    it('defaults to the public schema', async () => {
      ...
      await validateSequelizeSchema(sequelize);
  ...
  describe('compareNullability', () => {
    ...
    it('treats a primary key as implicitly NOT NULL', () => {
      ...
      assert.strictEqual(compareNullability(false, true, true), 'code-stricter');
    });

    it('lets an explicit allowNull:true outrank the primary-key inference', () => {
      // Sequelize marks both foreign keys of a @BelongsToMany through-table as
      // primaryKey:true AND allowNull:true. That describes association wiring, not
      // a NOT NULL constraint, so it must not be read as one.
      assert.strictEqual(compareNullability(true, true, true), 'ok');
    });
  ...
  describe('nullability through validateSequelizeSchema', () => {
    ...
    it('does not flag the nullable foreign keys of a BelongsToMany through-table', async () => {
      // Regression, reproducing ChargingStationNetworkProfile: neither foreign key
      // declares allowNull, both are optional on the model, and both are nullable
      // in the database. Sequelize nonetheless marks them primaryKey:true (from the
      // BelongsToMany wiring) and allowNull:true (from the @BelongsTo on the
      // through-model itself), which used to read as "the model declares NOT NULL".
      @Table({ tableName: 'StationProfiles', timestamps: false })
      ...
      class Station extends Model {
        ...
        @BelongsToMany(() => Profile, () => StationProfile)
      ...
      class Profile extends Model {
        ...
        @BelongsToMany(() => Station, () => StationProfile)
        declare stations?: Station[];
      }

      const intCol = { data_type: 'integer', udt_name: 'int4', character_maximum_length: null };
      const findings = await findingsFor(
        [Station, Profile, StationProfile],
        [
          col({ table_name: 'Stations', column_name: 'id', is_nullable: 'NO', ...intCol }),
          col({ table_name: 'Profiles', column_name: 'id', is_nullable: 'NO' }),
          // Nullable in the database, matching the optional model properties.
          col({
            table_name: 'StationProfiles',
            column_name: 'stationId',
            is_nullable: 'YES',
            ...intCol,
          }),
          col({ table_name: 'StationProfiles', column_name: 'profileId', is_nullable: 'YES' }),
        ],
      );

      // Guards the premise: without this combination the test would pass for the
      // wrong reason if a Sequelize upgrade stopped injecting these.
      const attributes = StationProfile.getAttributes() as Record<
        string,
        { primaryKey?: boolean; allowNull?: boolean }
      >;
      assert.strictEqual(attributes.stationId.primaryKey, true);
      assert.strictEqual(attributes.stationId.allowNull, true);

      assert.deepStrictEqual(findings, [], JSON.stringify(findings));
    });

    it('still flags a nullable column under a primary key that does not say allowNull', async () => {
      // The narrow exemption above must not disarm real primary keys. The other
      // through-tables in this codebase (ComponentVariable and friends) land here:
      // primaryKey:true with allowNull undefined, backed by a real composite key.
      @Table({ tableName: 'Widgets', timestamps: false })
      ...
      const findings = await findingsFor(
        [KeyedWidget],
        [col({ column_name: 'id', is_nullable: 'YES' })],
      );
      assert.deepStrictEqual(kinds(findings), ['nullability-code-stricter']);
      assert.strictEqual(findings[0].severity, 'error');
    });
  ...
  describe('assertSequelizeSchemaMatches', () => {
    ...
    it('throws on drift so startup aborts', async () => {
      await expect(
        assertSequelizeSchemaMatches(gateHarness(driftedRows), baseConfig as any, logger as any),
      ).rejects.toThrow(SchemaValidationError);
    });

    it('carries the full report on the thrown error', async () => {
      const error = await assertSequelizeSchemaMatches(
 ...
 * and asserts validateSequelizeSchema() returns zero errors against the real 52 models.
